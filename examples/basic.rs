...
fn main() -> Result<()> {
    ...
    let evidence = graph.add_node(CreateNode::new(
        "Borrow checker",
    ...
    // Structured filter
    let claims = graph.find_nodes(&NodeFilter::new().node_type(NodeType::Claim))?;
    ...
    for step in &chain {
        println!(
            "  depth {}: {} ({:?})",
            step.depth, step.label, step.node_type
        );
    }
